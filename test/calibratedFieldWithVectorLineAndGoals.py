...
def calculate_conversion_factor(contours_red):
...
while True:
    ...
    if conversion_factor is None and contours_red:
        calculate_conversion_factor(contours_red)
    ...
    if contours_pink:
        ...
        if circles is not None:
            ...
            for (x, y, r) in circles:
                if x >= goal_left[0] - 12 and x <= goal_right[0] + 12 and calculate_distance(get_obstacle_center(),
                                                                                             (x, y)) >= 171:
                    num_balls += 1
